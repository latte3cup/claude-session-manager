...
export default function FileExplorer({
  ...
  const contextMenuItems = useMemo((): ContextMenuEntry[] => {
    ...
    if (entry.type === "folder") {
      ...
      items.push({ label: "Copy Name", icon: <CtxCopyIcon />, onClick: () => { void copyToClipboard(entry.name); closeContextMenu(); } });
      items.push({ label: "Copy Full Path", icon: <CtxCopyIcon />, onClick: () => { void copyToClipboard(fullPath); closeContextMenu(); } });
      items.push("separator");
